...
import { sizeGuideData, Tab, Category } from '@/constants/data' // adjust path as needed
...
export default function SizeGuideModal({ isOpen, onClose }: SizeGuideModalProps) {
  ...
  const [activeCategory, setActiveCategory] = useState<Category>('apparel');

  if (!isOpen) return null;

  const currentData = sizeGuideData[activeTab][activeCategory];
      ...
      <div className="relative w-full max-w-lg max-h-[90vh] bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col z-10">
        ...
        <div className="overflow-y-auto flex-1 bg-neutral-100 px-6 py-5">
          ...
          <div className="overflow-x-auto">
            <ApparelTable headers={currentData.headers} rows={currentData.rows} />
...
function ApparelTable({ headers, rows }: { headers: string[]; rows: string[][] }) {
  return (
    <table className="w-full text-sm text-neutral-600">
      <thead>
        <tr className="border-b border-neutral-200">
          {headers.map((h) => (
            <th key={h} className="py-3 text-left text-[10px] font-medium uppercase tracking-widest text-neutral-400">
              {h}
            </th>
          ))}
        </tr>
      ...
      <tbody className="divide-y divide-neutral-200">
        {rows.map((row) => (
          <tr key={row[0]} className="hover:bg-white transition-colors">
            <td className="py-3 font-semibold text-neutral-900 text-[13px]">{row[0]}</td>
            {row.slice(1).map((cell, i) => (
              <td key={i} className="py-3 text-[13px]">{cell}</td>
            ))}
          </tr>
        ))}
      </tbody>
