...
export default function SearchBar({
  ...
  useEffect(() => {
    setMounted(true); // ← fix 2: only runs on client
  }, []);
  ...
  // ← fix 3: early return BEFORE main return, OUTSIDE JSX
  if (!mounted) {
    return (
      <div
        role="button"
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className={`absolute ${align === "left" ? "left-0" : "right-0"} top-8 w-[300px] max-w-[calc(100vw-2rem)] bg-[#FAF8F4] border border-[#e5e1da] rounded-xl z-50 overflow-hidden shadow-sm`}
          >
            {/* Header */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-[#e5e1da]">
              <span className="text-[10px] font-bold uppercase tracking-widest text-[#b8502e] flex items-center gap-1.5">
                <TbSparkles size={11} />
                Search
              </span>
            ...
            <div className="px-4 pt-3 pb-2">
              <div className="relative flex items-center">
                ...
                <input
                  ...
                  className="w-full bg-white border border-[#e5e1da] rounded-lg pl-8 pr-8 py-2 text-[12px] text-[#111] placeholder:text-neutral-400 outline-none focus:border-[#b8502e] transition-colors"
                />
                {query && (
                  <button
                    onClick={handleClear}
            ...
            <div className="px-4 pb-3 flex gap-1.5">
              ...
              <span className="ml-auto text-[9px] text-neutral-400 self-center">
                {mode === "groq"
                  ? "Exact match"
                  : mode === "vector"
                    ? "Semantic"
            ...
            <div className="max-h-[340px] overflow-y-auto">
              ...
              {/* Error */}
              {error && !isPending && (
                <p className="text-center text-[11px] text-red-400 py-6 px-4">
                  {error}
                </p>
              )}
              ...
              {!isPending && !error && hasSearched && results.length === 0 && (
                <div className="text-center py-8 px-4">
                  ...
                  <p className="text-[12px] font-semibold text-[#111] mt-0.5">
                    "{query}"
                  </p>
                  {mode !== "hybrid" && (
                    <button
                      onClick={() => setMode("hybrid")}
                      className="mt-3 text-[10px] text-[#b8502e] underline underline-offset-2 cursor-pointer"
                    >
                    ...
                    {results.map((product, i) => (
                      ...
                      >
                        <Link
                          ...
                          onClick={handleClose}
                          className="flex gap-3 px-4 py-3 hover:bg-[#f0ece4] transition-colors group"
                        >
                          {/* Image — prefer first colorway image, fall back to main image */}
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-[11px] font-medium text-[#111] truncate group-hover:text-[#b8502e] transition-colors">
                              {product.name}
                            </p>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              <span className="text-[11px] font-semibold text-[#b8502e]">
                              ...
                              {product.status === "sale" || product.isOnSale ? (
                                <span className="text-[8px] font-bold uppercase tracking-widest bg-[#b8502e] text-white px-1.5 py-0.5 rounded-full">
                                  Sale
                                </span>
                              ) : product.status === "new" || product.isNew ? (
                                <span className="text-[8px] font-bold uppercase tracking-widest bg-[#111] text-white px-1.5 py-0.5 rounded-full">
                                  New
                                </span>
                              ) : null}
                            </div>
            ...
            {results.length > 0 && !isPending && (
              <div className="px-4 py-2.5 border-t border-[#e5e1da] flex items-center justify-between">
